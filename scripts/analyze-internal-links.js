...
class InternalLinkAnalyzer {
  ...
  parseArticle(content, type, slug) {
    ...
    // Extract text content without markdown
  ...
  findSuggestionsForArticle(sourceId, sourceArticle) {
    ...
    // Limit keywords to check for performance
    const sourceText = sourceArticle.textContent.toLowerCase();
    ...
    for (const [targetId, targetArticle] of this.articles) {
      ...
      if (sourceText.includes(titleLower)) {
        const titleRegex = new RegExp(`\\b${this.escapeRegex(targetArticle.title)}\\b`, 'gi');
        const titleMatches = sourceArticle.textContent.match(titleRegex) || [];
        if (titleMatches.length > 0) {
          score += titleMatches.length * 5;
          matches.push({
            keyword: targetArticle.title,
            count: titleMatches.length,
            contexts: this.findContexts(sourceArticle.textContent, targetArticle.title, 1),
            isTitle: true
          });
        }
      }
      
      // Check top keywords only (limit to 10 for performance)
      const topKeywords = targetArticle.keywords.slice(0, 10);
      for (const keyword of topKeywords) {
        ...
        if (sourceText.includes(keywordLower)) {
          ...
          const contentMatches = sourceArticle.textContent.match(regex) || [];
          ...
          if (contentMatches.length > 0) {
            ...
            matches.push({
              ...
              contexts: this.findContexts(sourceArticle.textContent, keyword, 1)
            });
          }
        }
      }
